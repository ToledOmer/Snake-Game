...
//                                        if (i != 14) {
//                                            unsigned int r = 1;
//                                            data(k + numOfCylinder).set_visible(r, left_view);
//                                            data(k + numOfCylinder).set_visible(r, right_view);
//                                            meshes[k + numOfCylinder].captured = false;
...
//                                    }
                                    ...
                                    if (k == i - 1 && i != 14) {
                                        meshes[meshes.size() - 1].pointsValue =
                                                meshes[meshes.size() - 1].pointsValue * (-1);
...
//                       {
                    ...
                    for (int j = numOfCylinder; j < meshes.size(); j++) {
                        unsigned int r=1;
                        data(j).set_visible(r, left_view);
                        data(j).set_visible(r, right_view);
...
//                                    Eigen::RowVector3d(0, 0, 0)
            ...
            Eigen::RowVector3f Viewer::getRandomLocation() {
                ...
                return location *10+Eigen::RowVector3f(0,7,0);
            }
...
//                    userPoints=0;
                    char answer;
                if(!timePassed&&levelNumber == 3) {
                    player.freeAllBuffers();
                    player.BackIsPlaying = false;
                    player.playEnd();
                    ...
                    std::cout << "TIME PASSED!(120 seconds) TYPE YOUR CHOOSE:" <<std::endl;
                    std::cout << "(B) - START A NEW GAME FROM THE BEGINNING" <<std::endl;
                    std::cout << "(Q) - QUIT" <<std::endl;
                    std::cin >>  answer;
