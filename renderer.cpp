...
//          std::cout << core_list.back().id <<std::endl;
...
void Renderer::moveSpheres() {
    ...
    for(int index=14; index < scn->data_list.size();index++  ){
        ...
        if(scn->meshes[index].movesInCurrentDirection==35){
...
void Renderer::UpdateCore() {
    ...
    Eigen::Matrix3f rotationMat =  scn->getParentsRotationMatrixes(scn->numOfCylinder);
    core(right_view).camera_up = rotationMat * Eigen::Vector3f(0,0,1);
    core(right_view).camera_eye = rotationMat * Eigen::Vector3f(0,-1,0);
    Eigen::RowVector3f HeadCenter = -(scn->MakeTrans()*scn->getParentsTrans(scn->numOfCylinder-1)*scn->data(scn->numOfCylinder-1).MakeTrans()).col(3).head(3); //A.col(1);
    Eigen::Vector3f toAdd=  (scn->getNewRotationVector(scn->numOfCylinder)
                         * -0.83);

    HeadCenter += toAdd;

    core(right_view).camera_translation =HeadCenter;
